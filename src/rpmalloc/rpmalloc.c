...
static int
_rpmalloc_span_is_fully_utilized(const span_t* span) {
...
_rpmalloc_heap_global_finalize(heap_t* heap) {
	...
	heap_t* list_heap = (heap_t*)atomic_load_ptr(&_memory_heaps[list_idx]); //-V557
	if (list_heap == heap) {
		atomic_store_ptr(&_memory_heaps[list_idx], heap->next_heap); //-V557
	} else {
...
_rpmalloc_heap_initialize(heap_t* heap) {
	...
	size_t list_idx = heap->id % HEAP_ARRAY_SIZE;
	do {
		next_heap = (heap_t*)atomic_load_ptr(&_memory_heaps[list_idx]);  //-V557
		heap->next_heap = next_heap;
	} while (!atomic_cas_ptr(&_memory_heaps[list_idx], heap, next_heap));  //-V557
}
...
rpmalloc_initialize_config(const rpmalloc_config_t* config) {
	...
	if (_memory_medium_size_limit > MEDIUM_SIZE_LIMIT) //-V547
