...
static const pf_uuid_t epmv4_uuid = { 0xE1AF8308, 0x5D1F, 0x11C9, { 0x91, 0xA4, 0x08, 0x00, 0x2B, 0x14, 0xA0, 0xFA} };
// DEA00000-6C97-11D1-8271-{xxxxyyyyzzzz}

// xxxx Identifies an object instance within a physical device in case
// there are more than one instances, where
// Bit 0 – Bit 11 Lower part of the instance or node number
// Bit 12 – Bit 15 Identifies the interface
// 0x0 : Interface 0 (Default)
// ...
// 0xF : Interface 15
// See "I" within Formula(57)

// yyyy
// Identifies the Device ID as a vendor specific number
// for the device class

// zzzz
// Represents the Vendor ID as a central administrative
// number assigned by the responsible user organisation

static       pf_uuid_t device_pnet_uuid = { 0xDEA00000, 0x6C97, 0x11D1, { 0x82, 0x71, 0x11,0x22, 0x33, 0x44, 0x55, 0x66 } };
// DEA00001-6C97-11D1-8271-00A02442DF7D
static const pf_uuid_t pnet_io_device_uuid = { 0xDEA00001, 0x6C97, 0x11D1, { 0x82, 0x71, 0x00, 0xA0, 0x24, 0x42, 0xDF, 0x7D } };
// 8a885d04-1ceb-11c9-9fe8-08002b104860
static const pf_uuid_t pnet_data_representation_uuid = { 0x8a885d04, 0x1ceb, 0x11c9, {0x9f, 0xe8, 0x08, 0x00, 0x2b, 0x10, 0x48, 0x60 } };
  ...
  pf_session_info_t *p_sess)
{
  if (p_sess != NULL)
  {
    ...
    {
      LOG_DEBUG(PF_RPC_LOG, "RPC(%d): Released session ix %u\n", __LINE__, (unsigned)p_sess->ix);
      memset(p_sess, 0, sizeof(*p_sess));
      p_sess->in_use = false;
    }
    else
    {
      LOG_DEBUG(PF_RPC_LOG, "RPC(%d): Session not in use\n", __LINE__);
    }
  }
  else
  {
...
static void pf_session_release_all(
  pnet_t             *net)
{
  uint16_t                ix = 0;
  ...
  pf_ar_t *p_ar)
{
  ...
  {
    LOG_INFO(PNET_LOG, "RPC(%d): AR is NULL\n", __LINE__);
  }
}
...
//////////////////////////////////////////////////////////////////////////
static void pf_cmrpc_send_rpc_alarm(
  pnet_t   *net,
  void     *arg,
  uint32_t current_time)
{
  pnet_pnio_status_t alarm_data;
  alarm_data.error_code = PNET_ERROR_CODE_RTA_ERROR;
  alarm_data.error_decode = PNET_ERROR_DECODE_PNIO;
  alarm_data.error_code_1 = PNET_ERROR_CODE_1_RTA_ERR_CLS_PROTOCOL;
  alarm_data.error_code_2 = PNET_ERROR_CODE_2_ABORT_AR_RE_RUN_ABORTS_EXISTING;
  pf_alarm_send(net, &alarm_data);
  (void)current_time;
  (void)arg;
  ...
  uint16_t               *p_res_pos)
{
  ...
  else if (pf_ar_allocate(net, &p_ar) == 0)
  {
    /* Cross-reference */
    p_ar->p_sess = p_sess;
    /* prepare possible timeout error code */
    ...
    if (pf_cmrpc_rm_connect_interpret_ind(p_sess, &req_pos, p_ar) == 0)
    {
      ...
      {
        LOG_INFO(PNET_LOG, "CMRPC(%d): Connect uuid %s: Valid, unknown AR, NoArSet\n",
                 __LINE__,
                 pf_diag_get_uuid_as_string(&p_ar->ar_param.ar_uuid));
  ...
  {
    // still go through received data and check the AR UUID -> if the same, report error 0xDB 0x81 0x3D 0x00 and alarm 0xCF 0x81 253 14
    
    bool              b_error_set = false;
    pf_ar_t           ar_temp;            // allocate temporary AR on the stack
    memset(&ar_temp, 0, sizeof(ar_temp)); // must clear it and mark as is in use
    ar_temp.in_use = true;
    // cross reference
    ar_temp.p_sess = p_sess;
    p_sess->p_ar = &ar_temp;
    if (pf_cmrpc_rm_connect_interpret_ind(p_sess, &req_pos, &ar_temp) == 0)
    {
      if (pf_ar_find_by_uuid(net, &ar_temp.ar_param.ar_uuid, &p_ar) == 0)
      {
        LOG_WARNING(PF_RPC_LOG, "CMRPC(%d): Duplicate AR\n", __LINE__);
        (void)pf_cmdev_state_ind(net, p_ar, PNET_EVENT_ABORT);
        pf_set_error(&p_sess->rpc_result, PNET_ERROR_CODE_CONNECT, PNET_ERROR_DECODE_PNIO, PNET_ERROR_CODE_1_CMDEV, PNET_ERROR_CODE_2_CMDEV_STATE_CONFLICT);
        pf_scheduler_add(net, 2000, "alarm_send", pf_cmrpc_send_rpc_alarm, p_ar, &(net->rpc_alarm_timeout));
        b_error_set = true;
  ...
  uint16_t *p_res_pos)
{
  ...
  // need to response with an uuid, so extract it here
  // we need an uuid and control key, so extract it now to a separate block
  pf_control_block_t cblock;
  memset(&cblock, 0, sizeof(cblock));
  if (pf_cmrpc_rm_release_interpret_req(p_sess, req_pos, &cblock) == 0)
  {
    release_io.ar_uuid     = cblock.ar_uuid;
    release_io.session_key = cblock.session_key;
  }
  ...
  pf_control_block_t     *p_control_io)
{
  ...
  if (ret == 0)
  {
    if (p_sess->get_info.result == PF_PARSE_OK)
    {
      ...
      {
        LOG_WARNING(PF_RPC_LOG, "CMRPC(%d): ControlCommand has more than one bit set: %04x\n", __LINE__, p_control_io->control_command);
        ...
        // Automated RT tester here wants error code 0xDD 0x81 0x14 0x08 
        // don't know the symbolic name for error2 = 0x8
        pf_set_error(&p_sess->rpc_result, PNET_ERROR_CODE_CONTROL, PNET_ERROR_DECODE_PNIO, PNET_ERROR_CODE_1_DCTRL_FAULTY_CONNECT, 0x8); 
        ret = -1;
      }
    }
    else
      ...
      (p_sess->get_info.result == PF_PARSE_OK))
  {
        ...
        (p_sess->get_info.result == PF_PARSE_OK))
    {
      ...
      default:
        ...
        LOG_INFO(PF_RPC_LOG, "CMRPC(%d): Unknown block type %u\n", __LINE__, block_header.block_type);
        break;
      }
    }
  }
  ...
  if (ret == 0)
  {
    if (p_sess->get_info.result == PF_PARSE_OK)
    {
      ...
      if (req_pos != p_sess->get_info.len)
      {
        ...
        LOG_INFO(PF_RPC_LOG, "CMRPC(%d): req_pos (%u) != p_sess->get_info.len %u\n", 
                 __LINE__, 
                 (uint32_t)req_pos, 
                 (uint32_t)p_sess->get_info.len);
        ret = -1;
      }
      else if ((uint32_t)(req_pos - start_pos) != p_sess->ndr_data.args_length)
      {
        LOG_INFO(PF_RPC_LOG, "CMRPC(%d): args_length %" PRIu32 " != request length %" PRIu32 "\n", __LINE__,
                  p_sess->ndr_data.args_length, (uint32_t)(req_pos - start_pos));
        pf_set_error(&p_sess->rpc_result, PNET_ERROR_CODE_READ, PNET_ERROR_DECODE_PNIO, PNET_ERROR_CODE_1_CMRPC, PNET_ERROR_CODE_2_CMRPC_ARGSLENGTH_INVALID);
        ret = -1;
      }
    ...
    else
    {
      pf_set_error(&p_sess->rpc_result, PNET_ERROR_CODE_READ, PNET_ERROR_DECODE_PNIO, PNET_ERROR_CODE_1_CMRPC, PNET_ERROR_CODE_2_CMRPC_ARGSLENGTH_INVALID);
      ret = -1;
      LOG_INFO(PF_RPC_LOG, "CMRPC(%d): p_sess->get_info.result == %u\n",
               __LINE__,
...
//**
// * @internal
// * Take a DCE RPC EPM request and create a response.
// * doc file: TCIS_00000020_IP_UDP_RPC_I&M_EPM_v2.35.3.00006
// * tables 18, 19, 20
// * @param net              InOut: The p-net stack instance
// * @param p_sess           In:   The session instance.
// * @param p_rpc            In:   The RPC header.
...
// * @return  0  if operation succeeded.
// *          -1 if an error occurred.
// ToDo: rewrite this function to not use hard-coded constants

static int pf_cmrpc_epm_ind(pnet_t *net,
                            pf_session_info_t *p_sess,
                            pf_rpc_header_t *p_rpc,
                            uint16_t req_pos,
                            uint16_t res_size,
...
{
  int ret = -1;
  uint16_t opnum = p_rpc->opnum;
  if (opnum == PF_RPC_DEV_OPNUM_READ)
  {    
    pf_get_info_t *p_info = &(p_sess->get_info);
    const bool is_big_endian = p_info->is_big_endian;
    pf_epmv4_req_t epmv4_req;
    pf_get_epmv4_req(p_info, &req_pos, &epmv4_req);

    pf_put_uint32(is_big_endian, epmv4_req.entry_handle_attribute, res_size, p_res, p_res_pos);
    
    if(memcmp(&(epmv4_req.entry_handle_uuid), &null_uuid, sizeof(pf_uuid_t)) == 0)
    {
      // null uuid is initial request, return our uuid
      pf_put_uuid(is_big_endian, &(device_pnet_uuid), res_size, p_res, p_res_pos);
    }
    else
    {
      // finish the requests by putting null
      pf_put_uuid(is_big_endian, &(null_uuid), res_size, p_res, p_res_pos);
    }

    pf_put_uint32(is_big_endian, 1, res_size, p_res, p_res_pos); // number of entries
    pf_put_uint32(is_big_endian, 1, res_size, p_res, p_res_pos); // max entries
    pf_put_uint32(is_big_endian, 0, res_size, p_res, p_res_pos); // entries offset
    pf_put_uint32(is_big_endian, 1, res_size, p_res, p_res_pos); // entries count
    pf_put_uuid(is_big_endian, &(device_pnet_uuid), res_size, p_res, p_res_pos); // object UUID
    
    pf_put_uint32(is_big_endian, 3, res_size, p_res, p_res_pos); // tower reference
    pf_put_uint32(is_big_endian, 0, res_size, p_res, p_res_pos); // annotation offset
    pf_put_uint32(is_big_endian, 0x40, res_size, p_res, p_res_pos); // annotation length
    ...
    pf_put_mem(temp_string, 26, res_size, p_res, p_res_pos); // device type + blank
    //          0123456789012345678901
    snprintf(temp_string,
             sizeof(temp_string),
             "%-20s ",
             net->fspm_cfg.im_0_data.order_id);
    pf_put_mem(temp_string, 21, res_size, p_res, p_res_pos); // order id + blank

    // works only for version of one digit (0 - 9)!
    snprintf(temp_string,
             sizeof(temp_string),
             "    %i ",
             net->p_fspm_default_cfg->im_0_data.im_hardware_revision);

    //         "    2 "
    pf_put_mem(temp_string, 6, res_size, p_res, p_res_pos); // hw rev + blank

    // works only for versions of one digit (0 - 9)!
    memset(temp_string, 0, sizeof(temp_string));
    snprintf(temp_string,
             sizeof(temp_string),
             "%c  %i  %i  %i",
             net->p_fspm_default_cfg->im_0_data.sw_revision_prefix,
             net->p_fspm_default_cfg->im_0_data.im_sw_revision_functional_enhancement,
             net->p_fspm_default_cfg->im_0_data.im_sw_revision_bug_fix,
    ...
    uint32_t rest = (*p_res_pos) & 0x3;
    if(rest != 0)
    {
      rest = 4 - rest;
      while (rest > 0)
      {
        pf_put_byte(0, res_size, p_res, p_res_pos); // padding
        rest--;
      }
    }

    pf_put_uint32(is_big_endian, 75, res_size, p_res, p_res_pos); // tower length, value from Siemens code
    pf_put_uint32(is_big_endian, 75, res_size, p_res, p_res_pos); // tower octet string length

    pf_put_uint16(false, 5, res_size, p_res, p_res_pos); // floor count in little endian

    // index 0 - interface-id: 19 bytes: 0x0D, uuid, interface version major
    pf_put_uint16(false, 19, res_size, p_res, p_res_pos); // byte count
    // left data
    pf_put_byte('\x0D', res_size, p_res, p_res_pos); // rpcid
    pf_put_uuid(false, &(pnet_io_device_uuid), res_size, p_res, p_res_pos);
    pf_put_uint16(false, 1, res_size, p_res, p_res_pos); // version
    // right data
    pf_put_uint16(false, 2, res_size, p_res, p_res_pos); // byte count
    pf_put_uint16(false, 0, res_size, p_res, p_res_pos); // version minor

    // index 1 - data-rep: 19 bytes: 0x0D, uuid, interface version major
    pf_put_uint16(false, 19, res_size, p_res, p_res_pos); // byte count
    // left data
    pf_put_byte('\x0D', res_size, p_res, p_res_pos); // rpcid
    pf_put_uuid(false, &(pnet_data_representation_uuid), res_size, p_res, p_res_pos);
    pf_put_uint16(false, 0x0002, res_size, p_res, p_res_pos); // note in doc is written 0x01, in Siemens code is 0x2
    // right data
    pf_put_uint16(false, 2, res_size, p_res, p_res_pos); // byte count
    pf_put_uint16(false, 0, res_size, p_res, p_res_pos); // ??

    // index 2 - protocol-id: 1 byte
    pf_put_uint16(false, 1, res_size, p_res, p_res_pos); // byte count
    // left data
    pf_put_byte('\x0A', res_size, p_res, p_res_pos); // protocol-id
    // right data
    pf_put_uint16(false, 2, res_size, p_res, p_res_pos); // byte count
    pf_put_uint16(false, 0, res_size, p_res, p_res_pos); // ??
    ...
    pf_put_uint16(false, 1, res_size, p_res, p_res_pos); // byte count
    // left data
    pf_put_byte('\x08', res_size, p_res, p_res_pos); // udp port
    // right data
    pf_put_uint16(false, 2, res_size, p_res, p_res_pos); // byte count
    pf_put_uint16(true, 0xC000, res_size, p_res, p_res_pos); // PN port >= 0xC000 in big endian

    // index 4 - host address: 1 byte
    pf_put_uint16(false, 1, res_size, p_res, p_res_pos); // byte count
    // left data
    pf_put_byte('\x09', res_size, p_res, p_res_pos); // host address
    // right data
    pf_put_uint16(false, 4, res_size, p_res, p_res_pos); // byte count
    pf_put_uint32(true, net->cmina_temp_dcp_ase.full_ip_suite.ip_suite.ip_addr, res_size, p_res, p_res_pos); // host address in big endian
...
//     // pad to 4 bytes
    ...
    //pf_put_byte(0, res_size, p_res, p_res_pos); // padding
    pf_put_uint32(is_big_endian, 0, res_size, p_res, p_res_pos); // rpc epmap status
...
static int pf_cmrpc_rm_read_ind(
  ...
  uint8_t           *p_res,
  uint16_t          *p_res_pos,
  const uint16_t     end_of_rpc_header_pos)
{
  int                     ret = -1;
  ...
  // EPMv4 lookup
  else if (memcmp(&p_rpc->interface_uuid, &epmv4_uuid, sizeof(pf_uuid_t)) == 0)
  {
    ret = pf_cmrpc_epm_ind(net, 
                           p_sess, 
                           p_rpc, 
                           end_of_rpc_header_pos, 
                           res_size,
                           p_res,
                           p_res_pos);
  }
  else if (pf_cmrpc_rm_read_interpret_ind(p_sess, req_pos, &read_request) == 0)
  {
        ...
        (opnum != PF_RPC_DEV_OPNUM_READ_IMPLICIT))
    {
      ...
      LOG_INFO(PF_RPC_LOG, "CMRPC(%d): error\n", __LINE__);
    }
    ...
    else
    {
      ...
      if (pf_cmrdr_rm_read_ind(net, p_ar, opnum, &read_request, &p_sess->rpc_result, res_size, p_res, p_res_pos) == 0)
      {
  ...
  bool                    is_new_session)
{
  ...
  case PF_RPC_DEV_OPNUM_CONNECT:
    LOG_DEBUG(PF_RPC_LOG, "CMRPC(%d): Incoming CONNECT request via DCE RPC on UDP\n", __LINE__);
    ret = pf_cmrpc_rm_connect_ind(net, p_sess, req_pos, res_size, p_res, p_res_pos);
    break;
  case PF_RPC_DEV_OPNUM_RELEASE:
    LOG_DEBUG(PF_RPC_LOG, "CMRPC(%d): Incoming RELEASE request via DCE RPC on UDP\n", __LINE__);
    ret = pf_cmrpc_rm_release_ind(net, p_sess, req_pos, res_size, p_res, p_res_pos);
    // release always
    p_sess->kill_session = true;
    break;
  case PF_RPC_DEV_OPNUM_READ:
    LOG_DEBUG(PF_RPC_LOG, "CMRPC(%d): Incoming READ request via DCE RPC on UDP\n", __LINE__);
    ret = pf_cmrpc_rm_read_ind(net, 
                               p_sess, 
                               p_rpc, 
                               req_pos, 
                               res_size, 
  ...
  case PF_RPC_DEV_OPNUM_WRITE:
    LOG_DEBUG(PF_RPC_LOG, "CMRPC(%d): Incoming WRITE request via DCE RPC on UDP\n", __LINE__);
    ret = pf_cmrpc_rm_write_ind(net, p_sess, req_pos, res_size, p_res, p_res_pos);
    break;
  case PF_RPC_DEV_OPNUM_CONTROL:
    LOG_DEBUG(PF_RPC_LOG, "CMRPC(%d): Incoming DCONTROL request via DCE RPC on UDP\n", __LINE__);
    ret = pf_cmrpc_rm_dcontrol_ind(net, p_sess, p_rpc, req_pos, res_size, p_res, p_res_pos);
    break;
  case PF_RPC_DEV_OPNUM_READ_IMPLICIT:
    LOG_DEBUG(PF_RPC_LOG, "CMRPC(%d): Incoming READ_IMPLICIT request via DCE RPC on UDP\n", __LINE__);
    ret = pf_cmrpc_rm_read_ind(net, 
                               p_sess, 
                               p_rpc, 
                               req_pos, 
  ...
  bool                   *p_is_release)
{
  ...
  const uint16_t end_of_rpc_header_pos = req_pos;
  ...
  else
  {
        ...
        (rpc_req.flags.last_fragment == true))
    {
      ...
      case PF_RPC_PT_REQUEST:
        ...
        // lookup of EPMv4, ndr data does not follow, use maximum buffer size for response
        if (   (rpc_req.opnum == PF_RPC_DEV_OPNUM_READ)
            && (memcmp(&(rpc_req.interface_uuid), &epmv4_uuid, sizeof(pf_uuid_t)) == 0))
        {
  ...
  pnet_t *net)
{
  ...
  for (ix = 0; ix < NELEMENTS(net->cmrpc_session_info); ix++)
  {
    if ((net->cmrpc_session_info[ix].in_use == true) && (net->cmrpc_session_info[ix].from_me == true))
    {
      ...
      if (dcerpc_req_len > 0)
      {
        ...
        LOG_DEBUG(PF_RPC_LOG, "CMRPC(%d): Received %u bytes UDP payload from remote port %u, on a socket in session with index %u\n", __LINE__, dcerpc_req_len, dcerpc_port, ix);
        is_release = false;
  ...
  {
    dcerpc_resp_len = PF_MAX_UDP_PAYLOAD_SIZE;
    LOG_DEBUG(PF_RPC_LOG, "CMRPC(%d): Received %u bytes UDP payload from remote port %u, on the socket for incoming DCE RPC requests.\n", __LINE__, dcerpc_req_len, dcerpc_port);
    is_release = false;
    (void)pf_cmrpc_dce_packet(net, dcerpc_addr, dcerpc_port, net->cmrpc_dcerpc_req_frame, dcerpc_req_len, net->cmrpc_dcerpc_rsp_frame, &dcerpc_resp_len, &is_release);
  ...
  dcerpc_req_len = os_udp_recvfrom(net->pnet_socket, &dcerpc_addr, &dcerpc_port, net->pnet_req_frame, sizeof(net->pnet_req_frame));
  if (dcerpc_req_len > 0)
  {
    dcerpc_resp_len = PF_MAX_UDP_PAYLOAD_SIZE;
    is_release = false;
    (void)pf_cmrpc_dce_packet(net, dcerpc_addr, dcerpc_port, net->pnet_req_frame, dcerpc_req_len, net->pnet_rsp_frame, &dcerpc_resp_len, &is_release);
    if (dcerpc_resp_len != 0)
    {
      os_udp_sendto(net->pnet_socket, dcerpc_addr, dcerpc_port, net->pnet_rsp_frame, dcerpc_resp_len);
    }
    else
    {
      LOG_INFO(PF_RPC_LOG, "CMRPC(%d): PNET UDP: Nothing to send!\n", __LINE__);
    }
    if (is_release == true)
    {
      LOG_INFO(PF_RPC_LOG, "CMRPC(%d): re-open PNET UDP\n", __LINE__);
      os_udp_close(net->pnet_socket);
      net->pnet_socket = os_udp_open(OS_IPADDR_ANY, OS_PF_PNET_SERVER_PORT);
    }
  }
  ...
  pnet_t *net)
{
  ...
  // save device id and vendor id to pnet_uuid
  device_pnet_uuid.data4[2] = 0; // instance or node number
  device_pnet_uuid.data4[3] = 1; // interface
  device_pnet_uuid.data4[4] = net->p_fspm_default_cfg->device_id.device_id_hi;
  device_pnet_uuid.data4[5] = net->p_fspm_default_cfg->device_id.device_id_lo;
  device_pnet_uuid.data4[6] = net->p_fspm_default_cfg->device_id.vendor_id_hi;
  device_pnet_uuid.data4[7] = net->p_fspm_default_cfg->device_id.vendor_id_lo;
}
