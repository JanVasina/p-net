  ...
  uint16_t   *p_pos)
{
  if (((*p_pos) + src_size) >= res_len)
  {
    /* Reached end of buffer */
    LOG_ERROR(PNET_LOG,
              "BW(%d): End of buffer reached, pos %u len %u\n",
              __LINE__,
              *p_pos,
              res_len);
    p_bytes = NULL;
...
void pf_put_uint16(
  ...
  uint16_t  *p_pos)
{
#ifdef USE_BUILT_IN_SWAP
  if (*p_pos < res_len - sizeof(uint16_t))
  {
    if (is_big_endian)
    {
      *((uint16_t *)(&p_bytes[*p_pos])) = __builtin_bswap16(val);
    }
    else
    {
      *((uint16_t *)(&p_bytes[*p_pos])) = (val);
    }
...
{
#ifdef USE_BUILT_IN_SWAP
  if (*p_pos < res_len - sizeof(uint32_t))
  {
    if (is_big_endian)
    {
      *((uint32_t *)(&p_bytes[*p_pos])) = __builtin_bswap32(val);
    }
    else
    {
      *((uint32_t *)(&p_bytes[*p_pos])) = (val);
    }
...
{
#ifdef USE_BUILT_IN_SWAP
  if (*p_pos < res_len - sizeof(uint64_t))
  {
    if (is_big_endian)
    {
      *((uint64_t *)(&p_bytes[*p_pos])) = __builtin_bswap64(val);
    }
    else
    {
      *((uint64_t *)(&p_bytes[*p_pos])) = (val);
    }
  ...
  if (is_big_endian)
  {
    pf_put_byte((val >> 56ULL) & 0xff, res_len, p_bytes, p_pos);
    pf_put_byte((val >> 48ULL) & 0xff, res_len, p_bytes, p_pos);
    pf_put_byte((val >> 40ULL) & 0xff, res_len, p_bytes, p_pos);
    pf_put_byte((val >> 32ULL) & 0xff, res_len, p_bytes, p_pos);
    pf_put_byte((val >> 24ULL) & 0xff, res_len, p_bytes, p_pos);
    pf_put_byte((val >> 16ULL) & 0xff, res_len, p_bytes, p_pos);
    pf_put_byte((val >>  8ULL) & 0xff, res_len, p_bytes, p_pos);
    pf_put_byte(val & 0xff, res_len, p_bytes, p_pos);
  }
  else
  {
    pf_put_byte(val & 0xff, res_len, p_bytes, p_pos);
    pf_put_byte((val >>  8U) & 0xff, res_len, p_bytes, p_pos);
    pf_put_byte((val >> 16U) & 0xff, res_len, p_bytes, p_pos);
    pf_put_byte((val >> 24U) & 0xff, res_len, p_bytes, p_pos);
    pf_put_byte((val >> 32U) & 0xff, res_len, p_bytes, p_pos);
    pf_put_byte((val >> 40U) & 0xff, res_len, p_bytes, p_pos);
    pf_put_byte((val >> 48U) & 0xff, res_len, p_bytes, p_pos);
    pf_put_byte((val >> 56U) & 0xff, res_len, p_bytes, p_pos);
}
#endif // USE_BUILT_IN_SWAP
}
  ...
  uint16_t *p_pos)
{
  ...
  */
  ...
  {
    if (p_ar == NULL) // always one AR for implicit read
    {
      cnt = 1;
...
//         if (   (p_ar_tmp != NULL) 
//             && (memcmp(&(p_ar_tmp->ar_param.ar_uuid), &null_uuid, sizeof(pf_uuid_t) != 0)))
//         {
//           cnt++;
//         }
//       }
    ...
    if (p_ar == NULL)
    {
      ...
      //for (ix = 0; ix < PNET_MAX_AR; ix++)
      {
        p_ar_tmp = pf_ar_find_by_index(net, 0);
        if (p_ar_tmp != NULL)
        {
  ...
  uint16_t               *p_pos)
{
  ...
  if (pf_cmdev_get_default_device(net, &p_device) == 0)
  {
  ...
  uint16_t *p_pos)
{
  ...
  pf_cmdev_get_default_device(net, &p_device);

  /*
   * ToDo: Walk the device tree:
   * If we encounter a sub-slot that contains I&M data then add it here.
   */
  pf_put_ident_device(is_big_endian, PF_BT_EXPECTED_IDENTIFICATION_DATA,
...
{
  const uint64_t secs = ((uint64_t)(ts->sec_hi) << 32U) + (uint64_t)(ts->sec_lo);
  return (secs * 1000000000ULL) + ((uint64_t)(ts->nano_sec));
}
  ...
  uint16_t      *p_pos)
{
  ...
  /* Insert block header for the write operation */
  // using version 1.0 of the logbook
  pf_put_block_header(is_big_endian,
  ...
  // documentation sometimes says timestamp has 12 bytes, sometimes 8 bytes
  // for logbook it is expected timestamp of 8 bytes  
  // version 1.0 uses actual time stamp, so get it now
  os_get_current_timestamp(&(p_log_book->time_ts));
  const uint64_t ts = timestamp_to_ns(&(p_log_book->time_ts));
  pf_put_uint64(is_big_endian, ts, res_len, p_bytes, p_pos);
  pf_put_uint16(is_big_endian, cnt, res_len, p_bytes, p_pos);

  while (ix != p_log_book->put)
  {
    const uint64_t entry_ts = timestamp_to_ns(&(p_log_book->entries[ix].time_ts));

    // pf_put_time_timestamp(is_big_endian, &p_log_book->entries[ix].time_ts, res_len, p_bytes, p_pos);
    pf_put_uint64(is_big_endian, entry_ts, res_len, p_bytes, p_pos);
