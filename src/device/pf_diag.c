  ...
  else if (pf_cmdev_get_device(net, &p_dev) == 0)
  {
    ...
    if (p_subslot != NULL)
    {
      ...
      if (p_item != NULL)
      {
        ...
        {
          p_item->usi = usi; // PF_USI_QUALIFIED_CHANNEL_DIAGNOSIS;
//           if(usi == PF_USI_QUALIFIED_CHANNEL_DIAGNOSIS)
//           {
...
//           }
        ...
        if (ret == 0)
        {
          ...
          if (overwrite == true && b_send_alarm == true)
          {
  ...
  bool                    b_send_alarm)
{
  ...
  else if (pf_cmdev_get_device(net, &p_dev) == 0)
  {
    ...
    {
      if (pf_cmdev_get_diag_item(net, item_ix, &p_item) == 0 && b_send_alarm)
      {
