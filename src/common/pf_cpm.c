...
// };
// static const pnet_ethaddr_t broadcast_mac_addr = {
...
#if PNET_PROFILE != 0
  const uint32_t exec = (uint32_t)(os_get_current_time_us() - start);
...
//       LOG_ERROR(PNET_LOG, "%s(%i) err_code %d\n", __FILE__, __LINE__, p_iocr->p_ar->err_code);
  ...
  case PF_CPM_STATE_RUN:
    ...
    /* 20, 21 */
//    if(frame_structure && (data_valid || backup))
//    {
//      atomic_init(&p_cpm->dht, 0U);
  ...
  if (pf_cpm_get_ar_iocr_desc(net, api_id, slot_nbr, subslot_nbr, &p_ar, &p_iocr, &p_iodata) == 0)
  {
    ...
    case PF_CPM_STATE_RUN:
      ...
      else
      {
        ...
        {
          os_mutex_lock(net->cpm_buf_lock);
          if ((p_iodata->data_length > 0) && (p_data != NULL))
          {
